...
export type ParallelResult = { ok: boolean; patches: string[]; failures: string[] };
...
export async function runParallel(
  ...
  if (!providers.length) return { ok: false, patches: [], failures: ["no providers"] };
  ...
  async function runOne(item: WorkItem) {
    if (locks.has(item.file)) { queue.push(item); return; }
    locks.add(item.file);
    try {
      for (const p of providers) {
        const res = await p.review(item);
        if (res.ok && res.patch) { patches.push(res.patch); return; }
      }
