...
(async () => {
  ...
  const { patches, failures } = await runParallel(items, plan.lang || "ts", plan.maxConcurrency || 4);
  if (!patches.length) { console.error("No patches generated.", failures); process.exit(1); }
  ...
  } catch { console.error("Patch failed --check"); process.exit(1); }
  ...
  try {
    ...
    console.error("Verifier failed. Please inspect and re-run."); process.exit(1);
  }
