...
(async () => {
  ...
  if (res.suggested_patch_unified && task.repoPath) {
    const patchPath = node_path_1.default.join(
      task.repoPath,
      ".patches",
      "codex-suggested.patch",
    );
    node_fs_1.default.mkdirSync(node_path_1.default.dirname(patchPath), {
      recursive: true,
    });
    node_fs_1.default.writeFileSync(
      patchPath,
      res.suggested_patch_unified.replace(/\r\n/g, "\n"),
      "utf8",
    );
