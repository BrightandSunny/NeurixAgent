import { Agent, run, fileSearchTool, webSearchTool, defineOutputGuardrail } from '@openai/agents';
import { SpecSchema } from '../../shared/schemas/spec';

const ssot = fileSearchTool({ vectorStoreId: process.env.NEURIX_SSOT_VECTOR ?? 'dev' });
...
export async function runNeurixFlow(text: string) {
  ...
  const route = /spec|research|code|docs/i.exec(String(r.finalOutput))?.[0]?.toLowerCase() ?? 'spec';
  if (route !== 'spec') {
