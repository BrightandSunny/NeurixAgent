...
function post(title: string, body: string) {
  execSync(
    `yarn -s tsx console/post.ts ${JSON.stringify(title)} ${JSON.stringify(body)}`,
    { stdio: "inherit" },
  );
}
...
if (!assistant || !thread) {
  console.warn("NEURIX_ASSISTANT_ID/NEURIX_THREAD_ID not set; skipping post.");
  process.exit(0);
}
if (!fs.existsSync(resPath)) {
  post("[Codex] run", "No result file found.");
  process.exit(0);
}
const res = JSON.parse(fs.readFileSync(resPath, "utf8"));
const task = fs.existsSync(taskPath)
  ? JSON.parse(fs.readFileSync(taskPath, "utf8"))
  : {};
const patchPath = task.repoPath
  ? path.join(task.repoPath, ".patches", "codex-suggested.patch")
  : null;
const summary = [
